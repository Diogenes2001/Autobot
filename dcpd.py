...
column_names = ['LATITUDE', 'LONGITUDE', 'AGE', 'ANNUAL_KILOMETERS', 'DAILY_KILOMETERS', 'YEARS_LICENSED', 'VEHICLE_YEAR',
                "NUMBER_OF_DRIVERS", "NUMBER_OF_VEHICLES", 'DEDUCTIBLE_DCPD',
                'AT_FAULT_CLAIMS', 'NOT_AT_FAULT_CLAIMS', "MINOR_CONVICTIONS", "MAJOR_CONVICTIONS", "SERIOUS_CONVICTIONS",
                "GENDER","DRIVER_MARTIAL_STATUS", 'VEHICLE_AGE', 'DRIVER_TRAINING_IND',
                'INCURRED_LOSS_DCPD']
...
dataset = dataset[dataset['INCURRED_LOSS_DCPD'] > 0]
...
""" sns.pairplot(train_dataset[["INCURRED_LOSS_DCPD", "AGE", "VEHICLE_YEAR"]], diag_kind="kde")
plt.show() """

train_stats = train_dataset.describe()
train_stats.pop("INCURRED_LOSS_DCPD")
train_stats = train_stats.transpose()
train_stats


train_labels = train_dataset.pop('INCURRED_LOSS_DCPD')
test_labels = test_dataset.pop('INCURRED_LOSS_DCPD')
...
plt.xlabel('True Values [INCURRED_LOSS_DCPD]')
plt.ylabel('Predictions [INCURRED_LOSS_DCPD]')
plt.axis('equal')
plt.axis('square')
