...
column_names = ['LATITUDE', 'LONGITUDE', 'AGE', 'DAILY_KILOMETERS', 'YEARS_LICENSED', 'VEHICLE_YEAR',
                'DEDUCTIBLE_COLLISION', 'AT_FAULT_CLAIMS', 'NOT_AT_FAULT_CLAIMS', 
                "MINOR_CONVICTIONS", "MAJOR_CONVICTIONS", "SERIOUS_CONVICTIONS",
...
dataset = dataset[dataset['INCURRED_LOSS_COLLISION'] > 0]
...
training= dataset.pop('DRIVER_TRAINING_IND')
dataset['YES'] = (training == 'Y')*1.0
dataset['NO'] = (training == 'N')*1.0
...
""" sns.pairplot(train_dataset[["INCURRED_LOSS_COLLISION", "AGE", "VEHICLE_YEAR"]], diag_kind="kde")
plt.show() """

train_stats = train_dataset.describe()
train_stats.pop("INCURRED_LOSS_COLLISION")
train_stats = train_stats.transpose()
train_stats


train_labels = train_dataset.pop('INCURRED_LOSS_COLLISION')
test_labels = test_dataset.pop('INCURRED_LOSS_COLLISION')
...
plt.xlabel('True Values [INCURRED_LOSS_COLLISION]')
plt.ylabel('Predictions [INCURRED_LOSS_COLLISION]')
plt.axis('equal')
plt.axis('square')
